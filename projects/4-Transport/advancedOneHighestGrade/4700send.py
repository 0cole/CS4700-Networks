...
class Sender:
    ...
    def send(self, message, retransmission=False):
        if retransmission:
            self.log(f"Retransmitting packet with seq {message['seq']}")
        else:
            ...
            if self.dup_ack_count >= 3:
                ...
                if missing_seqs:
                    ...
                    if seq_num in self.packets:
                        ...
                        self.send(packet_info['packet'], retransmission=True)
    ...
    def run(self):
        ...
        while True:
            ...
            while self.seq_num < self.send_base + int(self.cwnd) and not data_finished:
                ...
                self.send(packet)  # Initial transmission
